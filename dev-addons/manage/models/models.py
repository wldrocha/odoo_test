...
class task(models.Model):
    ...
    name = fields.Char(
        string="Nombre", required=True, help="Escribe tu nombre"
    )  # task name and label is "nombre"
    # under field.char can be added more attributes like:
    # readonly=True, required=True and help="help here".
    description = fields.Text(
        string="Descripción"
    )  # task description and label is "descripción"
    creation_date = fields.Date(
        string="Fecha de creación", default=fields.Date.today
    )  # task creation date and label is "fecha de creación"
    start_date = fields.Date(
        string="Fecha de inicio"
    )  # task start date and label is "fecha de inicio"
    end_date = fields.Date(
        string="Fecha de fin"
    )  # task end date and label is "fecha de fin"
    is_paused = fields.Boolean(
        string="Pausada"
    )  # task is paused and label is "pausada"
    ...
    sprint = fields.Many2one(
        "manage.sprint", string="Sprint"
    )  # task sprint and label is "sprint"
    ...
    technology = fields.Many2many(
        comodel_name="manage.technology",
        string="Tecnología",
        relation="technologies_task",
        column1="task_id",
        column2="technology_id",
    )  # task technology and label is "tecnología"
...
class sprint(models.Model):
    ...
    name = fields.Char(
        string="Nombre", required=True, help="Escribe tu nombre"
    )  # sprint name and label is "nombre"
    description = fields.Text(
        string="Descripción"
    )  # sprint description and label is "descripción"
    duration = fields.Integer()  # sprint duration and label is "duración"
    start_date = fields.Datetime(
        string="Fecha de creación", default=fields.Date.today
    )  # sprint creation date and label is "fecha de creación"
    end_date = fields.Datetime(
        compute="_get_end_date", store=True, string="fin"
    )  # sprint end date and label is "fecha de fin"
